...
def run_solo_condition(
    ...
    pool = ProtoAxiomPool(num_agents=n_agents, consensus_threshold=0.5)
    mdl = MDLEngine()
    ...
    for round_num in range(1, n_rounds + 1):
        ...
        pool.detect_consensus(
            step=round_num * stream_length,
            environment_name="ModularArithmetic",
            stream_naive_bits=len(obs) * __import__('math').log2(7),
        )

        if len(pool.axioms) > 0 and consensus_round is None:
