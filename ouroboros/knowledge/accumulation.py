...
class SessionResult:
    ...
    rounds_to_best: int        # which round had the best MDL
    best_mdl_cost: float
    n_axioms_at_start: int     # KB size before this session
    n_axioms_at_end: int       # KB size after
    n_new_axioms: int          # axioms discovered THIS session
    elapsed_seconds: float
    prior_benefit: float       # improvement vs fresh start (bits saved)
...
class AccumulationRecord:
    ...
    mean_rounds_early: float = 0.0   # mean rounds in sessions 1-10
    mean_rounds_late: float = 0.0    # mean rounds in sessions 90-100
    convergence_speedup: float = 0.0  # late/early ratio
...
class SimpleAxiomKB:
    ...
    A full implementation would use SQLite (Day 14). This version
    is in-memory for simplicity and speed in the accumulation experiment.
    """
    ...
    def get_seeds_for_environment(
        ...
        """Get top-k expression strings for this environment type."""
    ...
    def get_all_seeds(self, top_k: int = 10) -> List[str]:
        """Get top-k expressions across all environments."""
...
class AccumulationRunner:
    ...
    def _run_single_session(
        ...
        n_axioms_start = 0 if fresh_start else self._kb.n_axioms
        obs = env.generate(self.stream_length)

        # Fresh start baseline (no KB)
        if session_id == 1 or fresh_start:
            fresh_router = HierarchicalSearchRouter(RouterConfig(
                beam_width=self.beam_width,
                max_depth=4,
                n_iterations=self.n_iterations,
                random_seed=session_id * 7,
            ))
            start = time.time()
            fresh_result = fresh_router.search(obs, alphabet_size=env.alphabet_size)
            fresh_cost = fresh_result.mdl_cost
        else:
            fresh_cost = float('inf')  # only measured once

        # Main session (with KB priors as neural prior warm-start)
        start = time.time()
        result = self._router.search(obs, alphabet_size=env.alphabet_size)
        elapsed = time.time() - start

        # Update neural prior if discovery was good
        if result.expr and result.mdl_cost < 500:
            self._router.update_prior(obs, result.expr, reward_bits=max(0, 500 - result.mdl_cost))

        # Save to KB
        n_new = 0
        if result.expr and result.mdl_cost < 400:
            expr_str = result.expr.to_string()
            if self._kb.add_axiom(expr_str, result.mdl_cost, env.name, session_id):
                n_new = 1

        prior_benefit = max(0.0, fresh_cost - result.mdl_cost) if fresh_cost < float('inf') else 0.0

        return SessionResult(
            session_id=session_id,
            environment_name=env.name,
            rounds_to_best=1,  # simplified — full version tracks round-by-round
    ...
    def run(self, experiment_id: str = "accum_exp_1") -> AccumulationRecord:
        ...
        for session_id in range(1, self.n_sessions + 1):
            ...
            # Vary seed slightly each session
    ...
    def save_record(self, record: AccumulationRecord, path: str) -> None:
        """Save the full accumulation record to JSON."""
